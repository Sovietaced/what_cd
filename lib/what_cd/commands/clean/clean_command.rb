...
module Escort
  class CleanCommand < ::Escort::ActionCommand::Base
    ...
    def run(dir)
      ...
      clean_dir = File.dirname(__FILE__)
      Dir["#{clean_dir}/plugins/*.rb"].each do |file| 
        require file 
        ...
        if plugin.class == Class
          begin
            ...
            puts "Skipping '#{file}' since it is not a valid plugin! It must include the clean_plugin mixin."
          end
        end
      end
      # # Handle files first
